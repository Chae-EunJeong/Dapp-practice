...
contract SaleAnimalToken {
    ...
    mapping(uint256 => uint256) public animalTokenPrices;   // animalTokenId를 입력하면 가격 나옴
    ...
    function purchaseAnimalToken(uint256 _animalTokenId) public payable {
        ...
        // 판매중인 배열에서 방금 구매한 토큰의 가격 0으로 만듦
        animalTokenPrices[_animalTokenId] = 0;

        for (uint256 i = 0; i < onSaleAnimalTokenArray.length; i++) {
            if (animalTokenPrices[onSaleAnimalTokenArray[i]] == 0) {
                onSaleAnimalTokenArray[i] = onSaleAnimalTokenArray[onSaleAnimalTokenArray.length - 1];  // 맨 뒤 index를 방금 제거한 토큰의 자리로 옮김
